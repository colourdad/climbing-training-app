...
// muscles[], timer{sec, mode?, label?}.
//
// Exercise generator functions accept (phase, deload) and return an array of
// exercise objects. buildSession() in sessions.js calls these with the
// appropriate phase context.
// ============================================================================
...
// ----------------------------------------------------------------------------
export const limitExercises = (phase, deload = false) => {
...
// ----------------------------------------------------------------------------
export const volumeExercises = (phase, deload = false) => {
  if (phase.id === 1) return [
...
// ----------------------------------------------------------------------------
export const techExercises = (phase, deload = false) => {
  if (phase.id === 1) return [
    ex({ id: 'warmup',          name: 'Warm-up',                     sets: '10 min',           rest: '—',       category: 'finger_prehab', notes: 'Easy movement on main wall.',                                                   muscles: ['fingers', 'shoulders'],                        timer: { sec: 0 } }),
...
// ----------------------------------------------------------------------------
export const homeAExercises = (phase, deload = false) => {
  if (phase.id === 1) return [
...
// ----------------------------------------------------------------------------
export const homeBExercises = (phase, deload = false) => {
  if (phase.id === 1) return [
...
// 30 min total. Band pull-aparts, ring rows, core, and mobility only;
// no weighted pulling.
// ----------------------------------------------------------------------------
export const lightHomeExercises = () => [
...
// Each test session lists which assessments to run that day.
// ----------------------------------------------------------------------------
export const testingExercises = (testDay) => {
...
// ----------------------------------------------------------------------------
// Exercise catalog (for search / reference)
// Unique per session-type + phase + exercise id.
// buildExerciseCatalog lives here because it depends on the exercise
// generators above; it imports buildSession from sessions.js.
// ----------------------------------------------------------------------------

// Exercise ids treated as housekeeping and hidden from the searchable catalog.
export const HOUSEKEEPING_IDS = new Set(['warmup', 'warmup_pyramid', 'cooldown', 'reflect']);

export function buildExerciseCatalog(buildSessionFn) {
  const seen = new Set();
  const catalog = [];
  // Defer PHASES import to avoid circular deps — caller passes buildSession
  // which already has phase context baked in. Instead we iterate session types
  // across all 4 phases by driving buildSessionFn directly.
  const PHASES_LOCAL = [
    { id: 1, name: 'Foundation' },
    { id: 2, name: 'Strength' },
    { id: 3, name: 'Power Endurance' },
    { id: 4, name: 'Performance' },
  ];
  const sessionTypes = ['limit', 'volume', 'tech', 'homeA', 'homeB', 'lightHome', 'testing'];
  PHASES_LOCAL.forEach(phase => {
    sessionTypes.forEach(t => {
      if (t === 'testing' && phase.id !== 4) return;
      const builds = t === 'testing'
        ? ['wed', 'thu', 'fri', 'sat'].map(td => buildSessionFn(t, phase, { testDay: td }))
        : [buildSessionFn(t, phase, { deload: false })];
      builds.forEach(s => {
...
export function getAllMuscles(buildSessionFn) {
  const set = new Set();
  buildExerciseCatalog(buildSessionFn).forEach(e => e.muscles.forEach(m => set.add(m)));
